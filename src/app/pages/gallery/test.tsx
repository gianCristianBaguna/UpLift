...
export const loadGalleryFromFolders = async (): Promise<GalleryItem[]> => {
  ...
  // ✅ Dynamically generate gallery data for each subfolder
  for (let index = 0; index < folderStructure.subfolders.length; index++) {
    ...
    // Expected local folder path
    const basePath = `/gallery/gallerypage/${subfolder.name}`

    // ✅ Dynamically create file URLs (you’ll just need to make sure these files exist)
    // e.g. /public/gallery/gallerypage/basketball/1.jpg, 2.jpg, 3.jpg
    const images = Array.from({ length: 15 }, (_, i) => `${basePath}/${i + 1}.jpg`)

    // Filter only existing images (optional — static builds can’t check existence)
    const validImages = images.filter((url) => url)

    if (validImages.length === 0) continue

    const itemCategories = [...folderStructure.category]
    if (["feeding", "drive", "basketball"].some((key) => subfolder.name.includes(key))) {
      itemCategories.push("Outreach")
    } else if (["school", "library"].some((key) => subfolder.name.includes(key))) {
      itemCategories.push("Education")
    } else if (["relief", "bohol"].some((key) => subfolder.name.includes(key))) {
      itemCategories.push("Relief")
    }

    galleryItems.push({
      id: `local-${subfolder.name}`,
      title,
